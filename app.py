...
with tab4:
    ...
    with st.container(border=True):
        ...
        # 共用參數
        p_min_v = col_a.number_input("最低日成交量 (張)", value=300, min_value=0)

        # 根據模式顯示不同的 Slider 並存入 params
        params = {'min_v': p_min_v}
        
        if strat_mode == "💎 量縮糾結":
            # 這裡修正了你原本 code 中 p_col2 未定義的錯誤
            p_gap = col_b.slider("均線糾結度 (%)", 1.0, 10.0, 5.0, help="數值越小，六線越黏")
            p_vol = col_c.slider("量縮窒息比", 0.1, 1.0, 0.6, help="今日量 / 20日均量")
            params['gap'] = p_gap
            params['vol_ratio'] = p_vol
        else:
            p_breakout = col_b.slider("突破量比 (倍數)", 2.0, 10.0, 3.5)
            params['breakout_vol'] = p_breakout
            col_c.info("🚀 帶量突破：抓取今日量能噴發且站上均線之標的")

    # C. 執行範圍
    scope = st.radio("掃描範圍", ["全台股", "雲端庫"], horizontal=True)
    
    if st.button("🏁 執行策略掃描", type="primary", use_container_width=True):
        # 取得標的名單
        if scope == "全台股":
            all_tickers = [f"{c}.TW" if i.market=="上市" else f"{c}.TWO" 
                           for c, i in twstock.codes.items() if c.isdigit() and len(c)==4]
        else:
            # 確保 db 已在上方讀取
            all_tickers = [f"{re.search(r'\d{4}', str(x)).group(0)}.TW" 
                           for x in db['標的'] if re.search(r'\d{4}', str(x))]
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for i, future in enumerate(concurrent.futures.as_completed(future_to_ticker)):
                ...
                if i % 30 == 0:
                    ...
                    status_txt.text(f"正在掃描第 {i+1} 檔標的...")
        
        # 整理結果
        st.session_state.v73_results = pd.DataFrame(hits)
        status_txt.success(f"掃描完成！共發現 {len(hits)} 檔標的符合「{strat_mode}」模式。")

    # D. 結果顯示區
    if st.session_state.v73_results is not None and not st.session_state.v73_results.empty:
        # 根據模式動態排序：量縮糾結排「糾結度」；帶量突破排「量比」
        sort_col = "糾結度(%)" if "糾結度(%)" in st.session_state.v73_results.columns else "量比"
        df_final = st.session_state.v73_results.sort_values(sort_col).reset_index(drop=True)
        
        st.write("---")
        st.info("💡 點擊下方表格標的，查看六色均線診斷圖。")
        ...
        if selection.selection.rows:
            target_idx = selection.selection.rows[0]
            target_stock = df_final.iloc[target_idx]
